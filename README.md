...
|------|------|
| `-a` | 包含完整文件内容（不截断） |
| `-e` | 启用 AI 分析模式（生成 commit message 提示词，**默认已启用**） |
| `-E` | 禁用 AI 分析模式（不添加 commit message 提示词） |
...
| `AI_GIT_LANG` | 界面语言（`zh` 或 `en`） | 自动检测系统语言 |
| `AI_GIT_NO_WRITE` | 设为 `1` 或 `true` 时不将报告写入文件 | 未设置（即默认写入） |
